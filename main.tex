...
%%construction
\textbf{Construction.} The trustworthiness score is a composite score that is calculated based on nine journalistic criteria. For each domain, the organization assigns a binary label (yes/no) per criterion. The trustworthiness score is then calculated as the sum of the weighted criteria. 
...
In the latest version of the dataset, the criterion ``Avoids deceptive headlines'' was the most respected, and 8,597 of the sources satisfied it (78.9 \% of sources), followed by ``Does not repeatedly publish false content'' (77.5 \% of sources). Across all dataset versions, when one of these criteria is satisfied, the other one tends to as well ( Pearson's r = 0.88). The two criteria are the strongest indicators for a low rating. 

Across all dataset versions, the most common combination of values is that all criteria are satisfied (16.1\% of ratings), given that a majority of sources are considered trustworthy. The second most common combination is that all criteria are satisfied except for the criterion ``Does not repeatedly publish false content'' (XY\% of sources).
