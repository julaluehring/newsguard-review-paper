...
NewsGuard regularly adds new sources to the database but rarely removes any, i.e., the database is growing. Removed sources tend to have lower trustworthiness scores in some years and appear to be outlets with a lower reach. 
% Zooming in on the type of sources added to the database, they tend to have a lower score. This is likely due to the fact that the organization has been adding untrustworthy domains. 
...
%%construction
\textbf{Construction.} The trustworthiness score is a composite score that calculated based on nine journalistic criteria. For each domain, the organization assigns a binary label (yes/no) per criterion. The trustworthiness score is then calculated as the sum of the weighted criteria. 
